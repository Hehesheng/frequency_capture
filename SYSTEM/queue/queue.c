...
#include "queue.h"
/*******************************************************************************
* Function Name : uint32_t QUEUE_PacketCreate(QUEUE8_TYPE *pQ8, uint8_t *pBuf,
uint32_t lenSize)
* Description   : 队列建立
* Input         :   pQ8:    队列
                    pBuf:   队列缓冲区地址
...
/*******************************************************************************
* Function Name : uint32_t QUEUE_PacketIn(QUEUE8_TYPE *pQ8, uint8_t *pData,
uint32_t len)
* Description   : 数据载入队列
* Input         :   pQ8:    队列
                    pData:  要进队列的数据
...
/*******************************************************************************
* Function Name : uint32_t QUEUE_PacketOut(QUEUE8_TYPE *pQ8, uint8_t *pData,
uint32_t dataLen)
* Description   : 队列中取数据
* Input         :   pQ8:    队列
                    pData:  缓冲区
...
/*******************************************************************************
* Function Name : uint32_t QUEUE_PacketSplit(QUEUE8_TYPE *pQ8, uint8_t
startChar, uint8_t endChar, uint8_t *pData, uint32_t dataLen)
* Description   : 以起始符和结束符取队列中的数据 (取出的数据 包括起始符
和分隔符)
* Input         :   pQ8:        队列
                    startChar:  起始符
                    endChar:    结束符
...
* Date          : 2013.08.29
*******************************************************************************/
uint32_t QUEUE_PacketSplit(QUEUE8_TYPE *pQ8, uint8_t startChar, uint8_t endChar,
    ...
    index = 0;
    //获取从起始字符到结束字符的数据
    while ((pStart != pEnd) && (index < dataLen) && (index < pQ8->bufSize) &&
           count--) {
        pData[index++] = *pStart++;
        if (pStart >= pQ8->pBuf + pQ8->bufSize) pStart = pQ8->pBuf;
    }
...
/*******************************************************************************
* Function Name : uint32_t QUEUE_PacketCharSplit(QUEUE8_TYPE *pQ8, uint8_t
  splitChar, uint8_t *pData, uint32_t dataLen)
...
* Date          : 2013.08.30
*******************************************************************************/
uint32_t QUEUE_PacketDoubleByteSplit(QUEUE8_TYPE *pQ8, uint8_t splitChar,
                                     uint8_t *pData, uint32_t dataLen) {
    int32_t count;
    int32_t index;
    volatile uint8_t *pStart;
...
/*******************************************************************************
 * Function Name : uint32_t QUEUE_PacketCharSplit(QUEUE8_TYPE *pQ8, uint8_t
 * splitChar, uint8_t *pData, uint32_t dataLen) Description   :
 * 提取单结束分隔符的数据 (包括分隔符) Input         : Output        : Other :
 * Date          : 2013.10.20
 *******************************************************************************/
uint32_t QUEUE_PacketCharSplit(QUEUE8_TYPE *pQ8, uint8_t splitChar,
                               uint8_t *pData, uint32_t dataLen) {
    int32_t count;
    int32_t index;
    volatile uint8_t *pStart;
 ...
 * Date          :2014/03/27
 *******************************************************************************/
uint32_t QUEUE_PacketDoubleCharSplit(QUEUE8_TYPE *pQ8, uint8_t splitChar1,
                                     uint8_t splitChar2, uint8_t *pData,
                                     uint32_t dataLen) {
    int32_t count;
    int32_t index;
    volatile uint8_t *pStart;
